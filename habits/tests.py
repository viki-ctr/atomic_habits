...
class HabitModelTest(TestCase):
    def setUp(self):
        ...
        self.pleasant_habit = Habit.objects.create(
            user=self.user, place="Дом", time="08:00:00", action="Пить кофе", is_pleasant=True, duration=60
        )
    ...
    def test_invalid_duration(self):
        with self.assertRaises(ValidationError):
            habit = Habit(user=self.user, place="Офис", time="10:00:00", action="Работать", duration=121)
            habit.full_clean()
...
class HabitSerializerTest(APITestCase):
    ...
    def test_valid_serializer(self):
        serializer = HabitSerializer(
            data=self.habit_data, context={"request": type("obj", (object,), {"user": self.user})}
        )
    ...
    def test_invalid_duration(self):
        ...
        serializer = HabitSerializer(
            data=invalid_data, context={"request": type("obj", (object,), {"user": self.user})}
        )
...
class HabitViewSetTest(APITestCase):
    ...
    def test_create_habit(self):
        url = reverse("habits-list")
        data = {"place": "Офис", "time": "10:00:00", "action": "Работать", "duration": 90}
