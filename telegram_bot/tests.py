...
class TelegramHandlersTest(TestCase):
    ...
    def test_get_chat_id_success(self, mock_create_telegram_user, mock_authenticate):
        ...
        result = asyncio.run(self.run_async(get_chat_id(self.update, self.context)))
    ...
    def test_cancel_handler(self):
        ...
        result = asyncio.run(self.run_async(cancel(self.update, self.context)))
    ...
    def test_start_handler(self):
        ...
        asyncio.run(self.run_async(start(self.update, self.context)))
...
class TelegramTasksTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(email="test@example.com", password="testpass123")
        self.telegram_user = TelegramUser.objects.create(user=self.user, chat_id="12345")
        self.habit = Habit.objects.create(
            user=self.user, place="Дом", time="08:00:00", action="Пить воду", duration=60
        )
...
class TelegramWebhookTest(TestCase):
    def setUp(self):
        ...
        self.valid_data = {
            "update_id": 123,
            "message": {"message_id": 1, "date": 1630000000, "text": "Test", "chat": {"id": 1}},
        }
        self.invalid_data = "{bad: json}"

    def test_webhook_post_invalid_json(self):
        """Тест обработки невалидного JSON."""
        request = self.factory.post("/webhook/", data=self.invalid_data, content_type="application/json")
